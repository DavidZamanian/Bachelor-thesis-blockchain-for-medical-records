...
export function EHROverviewPVScreen(props) {
  ...
  const patientID =
    props.route.params == null ? 8701104455 : props.route.params;

  const placeholderPrescriptions = [
    "PollenStopper, 1 pill per day when needed",
    "NoseSpray, 1 dose in each nostril per day if needed",
  ];
  ...
  const [patientDiagnoses, setPatientDiagnoses] =
    useState(placeholderDiagnoses);
  const [patientRegions, setPatientRegions] = useState(
    placeholderPatientRegions
  );
    ...
    onValue(patientRef, (snapshot) => {
      if (snapshot.val() === null) {
        ...
        setRegions((prevState) => {
          patientRegions.forEach(
            (reg) => (prevState.find((r) => r.name === reg).enabled = true)
          );
                ...
                <View style={styles.journalContainer}>
                  <TouchableOpacity
                    style={[
                      ...
                      {
                        backgroundColor: journalExpanded[index]
