...
contract("Block4EHR", (accounts) => {
  ...
  before(async () => {
    ...
    await instance.addMedicalPersonnel(
      doc_boras_account,
      doc_boras,
      inst_boras
    );
    ...
    // ==== ADDING PATIENTS ====
    await instance.addPatient(pat_gbg_kungalv_accounts, pat_gbg_kungalv, [
      gbg,
      kungalv,
    ]);
  ...
  describe("updateEHR updates the patients EHR cid ", function () {
    it("updateEHR updates the patients EHR cid when called by permissioned personnel ", async function () {
      await instance.updateEHR(pat_gbg_kungalv, "123456", {
        from: doc_gbg_account,
      });
      let cid = await instance.getEHRCid(pat_gbg_kungalv, {
        from: doc_gbg_account,
      });
  ...
  describe("getEHRCid retrieves the cid conected to the latest EHR update", function () {
    it("getEHRCid retrieves the latest cid when called by the patient", async function () {
      let cid = await instance.getEHRCid(pat_gbg_kungalv, {
        from: pat_gbg_kungalv_accounts,
      });
      await instance.updateEHR(pat_gbg_kungalv, "678910", {
        from: doc_gbg_account,
      });
      let newcid = await instance.getEHRCid(pat_gbg_kungalv, {
        from: pat_gbg_kungalv_accounts,
      });
    ...
    it("getEHRCid retrieves the latest cid when called by a permitioned personnel", async function () {
      await instance.updateEHR(pat_boras, "3456789", {
        from: doc_boras_account,
      });
      let newcid = await instance.getEHRCid(pat_boras, {
        from: doc_boras_account,
      });
  ...
  describe("hasPermission returns the correct value", function () {
    it("hasPermission returns true for the patient itself", async function () {
      let res = await instance.hasPermission(pat_boras, {
        from: pat_boras_account,
      });
      assert.ok(res);
    });
  ...
  describe("getPermissionedRegion returns the correct result", function () {
    it("getPermissionedRegions returns the correct array of regions when called by the patient", async function () {
      let res = await instance.getPermissionedRegions(pat_gbg_kungalv, {
        from: pat_gbg_kungalv_accounts,
      });
      assert.ok(res);
    });
    ...
    it("getPermissionnedRegions throws an error when called by anybody else but the patient", async function () {
      let e;
      try {
        await instance.getPermissionedRegions(pat_gbg_kungalv, {
          from: doc_gbg_account,
        });
      } catch (error) {
        e = error;
  ...
  describe("setPermissions sets the permission correctly", function () {
    it("setPermissions sets the permission array correctly when the patient removes a region", async function () {
      let arr = [gbg];
      await instance.setPermissions(pat_gbg_kungalv, arr, {
        from: pat_gbg_kungalv_accounts,
      });
      let res = await instance.getPermissionedRegions(pat_gbg_kungalv, {
        from: pat_gbg_kungalv_accounts,
    ...
    it("setPermissions sets the permission array correctly when the patient adds a region", async function () {
      let arr = await instance.getPermissionedRegions(pat_boras, {
        from: pat_boras_account,
      });
      ...
      await instance.setPermissions(pat_boras, arr, {
        from: pat_boras_account,
      });
      let res = await instance.getPermissionedRegions(pat_boras, {
        from: pat_boras_account,
      });
      assert.deepEqual(res, arr);
    });
    ...
    it("setPermissions throws an error when anyone other than the patient invokes it", async function () {
      let e;
      try {
        await instance.setPermissions(pat_boras, [gbg], {
          from: doc_boras_account,
        });
      } catch (err) {
        e = err;
  ...
  describe("getHealthCareInstitution returns the Healthcare Institution the given medicall personnel works at", function () {
    ...
    it("getHealthCareInstitution returns empty Healthcare Institution object (empty strings as properties) given an invalid medical personel ", async function () {
      let healthcareInst = await instance.getHealthCareInstitution(
        "1111111111"
      );
      let healthcareInstId = healthcareInst.id;
